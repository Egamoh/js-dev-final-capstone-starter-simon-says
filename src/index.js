/*This is my second attempt at submitting the capstone. 
I chose a different project because I had gotten stuck on the Whack-a-Mole code. 
This Simon-Says project had its own set of issues. 
Namely, I was unable to run 'npm test', rather eliciting a 'run-p is not recognised as a internal or external command' error,
and necessitating that I judge whether I met the rubric by just playing the game (the dependencies install seem to have taken).*/
...
function padHandler(event) {
  ...
  // TODO: Write your code here.
  let target = color;
  let pad = pads.find((color) => target === color);
  pad.sound.play();
  checkPress(color);
  return color;
}
...
function setLevel(level = 1) {
  ...
  switch(level) {
    case 0:
      ...
      break;
    case 1:
      levelsTotal = 14; level ++;
      break;
    case 2:
      levelsTotal = 20; level ++;
      break;
    case 3:
      levelsTotal = 31; level ++;
      break;
    default:
      return "Please choose levels 1-4";
  }
  return levelsTotal;
}
// used w3Schools JS Switch chapter as guide
// I also referred to stackoverflow.com/questions/5464362/javascript-using-a-condition-in-switch-case
/**
...
function playHumanTurn() {
  ...
  setText(statusElement, `Player's remaining presses: ${maxRoundCount - roundCount + 1}`);
}
...
function checkRound() {
  ...
  if (playerSequence.length === maxRoundCount) {
    ...
    roundCount++;
    playerSequence = [];
    statusSpan.innerHTML = "Next Round";
